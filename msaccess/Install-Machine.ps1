...
if ($Stage -eq "SetupAgent" -or $Stage -eq "") {
    ...
    Invoke-Command -Session $session -ScriptBlock {
        ...
        @{
            ...
            VSO_AGENT_IGNORE = "AZP_AGENT_NAME,ChocolateyLastPathUpdate,chocolateyUseWindowsCompression,PROMPT"
            MsAccess = "$((Get-ChildItem 'C:/Program Files (x86)/Microsoft Office/Office14/MSACCESS.EXE').VersionInfo.FileVersion)"
            Nuget    = "$((Get-Command nuget).FileVersionInfo.FileVersion)"
        } |
            ConvertTo-Json |
